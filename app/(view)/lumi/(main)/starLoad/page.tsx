...
export default function Page() {
      ...
      try {
        ...
        if (template && template.points && template.points.length > 0) {
          ...
          const path = template.path_index
            ...
            : template.points;
          console.log("[starLoad] Anchor points:", path.length, path.slice(0, 3));
  ...
  const handleEdit = useCallback(() => {
    if (selectedDate) {
      router.push(`/lumi/write?date=${selectedDate}`);
    } else {
  ...
  const rangeDisplay = useMemo(() => {
    return `${displayDate(seasonRange.start)} ~ ${displayDate(seasonRange.end)}`;
  }, [seasonRange]);
          ...
          <div className="hero-inner p-4 min-h-[300px]">
            {loading ? (
              ...
              <div className="text-white/60 text-sm text-center py-8">
                별자리 데이터를 불러올 수 없습니다.
                <br />
                <span className="text-xs">daysCount: {seasonRange.daysCount}</span>
              </div>
