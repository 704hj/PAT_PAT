      ...
      if (signInError) {
        ...
        if (signInError.message.includes("Invalid login credentials")) {
          ...
          try {
            ...
            console.log("[useSignIn] Check response status:", checkResponse.status);
            ...
            if (checkResponse.ok) {
              ...
              if (checkData && checkData.exists === true) {
                ...
                console.log("[useSignIn] Setting error (password wrong):", errorMsg);
                setError(errorMsg);
              } 
              else {
                // 계정이 존재하지 않는 경우
                const errorMsg = "존재하지 않는 계정입니다. 다시 확인해주세요.";
                console.log("[useSignIn] Setting error (account not found):", errorMsg);
                setError(errorMsg);
              }
            } else {
              // API 호출 실패 시 일반적인 메시지
              const errorText = await checkResponse.text().catch(() => "");
              console.error("[useSignIn] Check API failed:", checkResponse.status, errorText);
