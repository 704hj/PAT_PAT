...
const ZODIAC_BY_MONTH: Record<number, string> = {
  1: '1_aquarius', 2: '2_pisces', 3: '3_aries',
  4: '4_taurus', 5: '5_gemini', 6: '6_cancer',
  7: '7_leo', 8: '8_virgo', 9: '9_libra',
  10: '10_scorpio', 11: '11_sagittarius', 12: '12_capricorn',
};
...
export default function HomeClient() {
  ...
  const weekStars = useMemo(() =>
    WEEK_LABELS.map((label, i) => {
      ...
      const isFilled = !isFuture && i >= (todayIndex - filledDays + (result?.isDiary ? 1 : 0)) && i <= todayIndex
        ? true
        : i < todayIndex && (todayIndex - i) <= filledDays - (result?.isDiary ? 1 : 0);
      ...
      <div className="relative min-h-[100svh] overflow-y-auto">
        ...
        <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
          <div style={{
            ...
            background: 'radial-gradient(ellipse, rgba(60,90,200,0.18) 0%, transparent 65%)',
            filter: 'blur(80px)',
          ...
          <div style={{
            ...
            top: 380, right: -60,
            background: 'radial-gradient(ellipse, rgba(80,40,180,0.1) 0%, transparent 70%)',
            filter: 'blur(70px)',
        ...
        {/* 배경 미세 별 */}
        {[
          [7, 11], [13, 83], [21, 93], [36, 5], [50, 89],
          [64, 16], [77, 71], [88, 43], [19, 57], [44, 67],
        ].map(([top, left], i) => (
          <div key={i}
        ...
        <section className="mx-auto w-full max-w-[480px] px-5 pb-[120px]">
            ...
            <div className="mt-5 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1.5 text-white/35 text-[13px] font-light">
                  {isDiary ? '오늘의 별을 남겼어요' : '오늘의 별을 기다리고 있어요'}
                </p>
              </div>
              <img
          ...
          <div
            ...
            style={{
              ...
              border: `1px solid ${isDiary ? 'rgba(140,175,255,0.15)' : 'rgba(255,255,255,0.07)'}`,
            }}
          >
            <div className="p-6">
              {/* 상태 + 별 */}
              <div className="flex items-start justify-between gap-4 mb-5">
                <div>
                  <p className="text-white/20 text-[10px] tracking-[0.25em] uppercase mb-3">Today</p>
                ...
                <div className="flex-shrink-0 mt-1">
                  <svg width="20" height="20" viewBox="0 0 24 24"
                    style={{
                      filter: isDiary ? 'drop-shadow(0 0 6px rgba(170,200,255,0.7))' : 'none',
                      animation: isDiary ? 'twinkle 3s ease-in-out infinite' : 'none',
                      opacity: isDiary ? 1 : 0.2,
                    }}
                  >
                    <polygon points={STAR_POINTS} fill="rgba(200,220,255,0.95)" />
                  </svg>
                </div>
              </div>
              ...
              <button
                onClick={() => router.push(
                  result?.diaryId ? `/diary/editor?diaryId=${result.diaryId}` : '/diary/editor'
                )}
                className="flex items-center gap-1.5 text-[13px] font-light transition-opacity duration-150 active:opacity-50"
                style={{ color: isDiary ? 'rgba(160,195,255,0.75)' : 'rgba(255,255,255,0.45)' }}
              >
                    ...
                    <div className="relative flex items-center justify-center" style={{ width: 32, height: 32 }}>
                      ...
                      <svg viewBox="0 0 24 24"
                        style={{
                          ...
                          position: 'relative',
                          filter: isFilled ? 'drop-shadow(0 0 4px rgba(160,200,255,0.6))' : 'none',
                          animation: isFilled && isToday ? 'twinkle 3s ease-in-out infinite' : 'none',
                      ...
                      >
                        <polygon
                          points={STAR_POINTS}
                          fill={isFilled ? 'rgba(195,218,255,0.92)' : 'rgba(255,255,255,0.14)'}
                        />
                      </svg>
                    </div>
          ...
          {collectedCount > 0 ? (
            ...
            >
              <div className="p-6 flex items-center justify-between">
                <div>
                  <p className="text-white/20 text-[10px] tracking-[0.25em] uppercase mb-3">Collection</p>
                  <p className="text-white/80 text-[17px] font-light">
                    <span className="text-white/90">{collectedCount}개</span>의 별자리를 수집했어요
                  </p>
                </div>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                  <circle cx="4" cy="18" r="2" fill="rgba(185,155,255,0.7)"
                    style={{ filter: 'drop-shadow(0 0 4px rgba(170,130,255,0.8))' }}
                  />
                  <circle cx="12" cy="7" r="2" fill="rgba(185,155,255,0.7)"
                    style={{ filter: 'drop-shadow(0 0 4px rgba(170,130,255,0.8))' }}
                  />
                  <circle cx="20" cy="14" r="2" fill="rgba(185,155,255,0.7)"
                    style={{ filter: 'drop-shadow(0 0 4px rgba(170,130,255,0.8))' }}
                  />
                  <line x1="4" y1="18" x2="12" y2="7" stroke="rgba(170,130,255,0.3)" strokeWidth="0.8" />
                  <line x1="12" y1="7" x2="20" y2="14" stroke="rgba(170,130,255,0.3)" strokeWidth="0.8" />
                </svg>
              </div>
            </div>
          ) : (
